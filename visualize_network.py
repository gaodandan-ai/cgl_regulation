...
def load_data():
    if not os.path.exists(REGULATIONS_FILE):
        print(f"Error: {REGULATIONS_FILE} not found. Please run this script in the workspace root.")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    if G is not None and G.number_of_nodes() > 0:
        ...
        # Save plots
        img_out = args.output_img if args.output_img else f"{resolved_query}_network.png"
        draw_matplotlib(G, resolved_query, img_out)
        
        if pyvis_available:
            html_out = args.output_html if args.output_html else f"{resolved_query}_network_interactive.html"
