...
- **Active Simulation & Biophysical Affinities (New in v0.3.0)**:
  ...
  - **Active ecFBA & dynamic rFBA Simulations**: Runs dynamic regulatory flux balance analysis and enzyme-constrained FBA (ecFBA) locally or via serverless fallbacks.
  ...
  - **Interactive Pathway Flow Diagram**: Visualizes Cytoscape.js-based compound-reaction metabolic network flows.
...
- **`data/model/iCW773.xml`**: Genome-scale metabolic model used for structural reaction mapping.
- **`data/reference/metabolic_models/`**: Enzyme-constrained metabolic model resource containing reaction parameters.
- **`data/reference/edge_confidence/`**: Feature matrices and trained Random Forest model files.
...
> - **Active Simulations & Fallbacks**: Standard FBA, FVA, rFBA, and ecFBA can be run dynamically. When running serverless (e.g., deployed on Vercel), the platform utilizes high-fidelity heuristic simulation fallbacks to display regulatory and pathway impacts. For complete local dynamic calculations, run the unified Python server backend locally.
> - **Validation Requirement**: Computational simulations and biophysical binding affinity scores serve as engineering prioritization guides. Wet-lab validation remains necessary before drawing definitive biological engineering conclusions.
...
Launch the static server and unified API controller:
...
│   └── index.py
├── backend/              # FastAPI FBA simulation backend
│   ├── app.py            # Main API routing definition
│   ├── simulation.py     # FBA, ecFBA, and rFBA algorithms
│   ├── model_loader.py   # cobrapy SBML model lazy-loader
│   ├── schemas.py        # Pydantic response models
│   └── models/           # Local metabolic model XML files
├── data/                 # Regulators, expressions, and metabolic datasets
│   ├── reference/        # Data assets bundled in deployment
│   ├── literature/       # Optional local literature files for RAG
│   └── mock_rnaseq.csv
...
│   ├── style.css         # UI CSS styling sheet
│   └── lib/              # Client libraries (Cytoscape, Chart.js, FontAwesome)
└── run_server.py         # Local launch script & API handler fallback
```
