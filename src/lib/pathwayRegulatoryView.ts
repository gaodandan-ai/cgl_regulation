import {
  ...
  getGenesForPathway as getAdapterGenesForPathway,
  normalizeGeneId
} from "./metabolicModelAdapter";

export type PathwayRegulatoryGene = {
  geneId: string;
  geneLabel?: string;
  reactions: {
    reactionId: string;
...
export type PathwayRegulator = {
  tfId: string;
  tfLabel?: string;
  regulatedGenes: string[];
  regulationTypes: ("activation" | "repression" | "predicted" | "unknown" | string)[];
  averageConfidence: number;
  regulatorScore: number;
  explanation: string;
};

export type PathwayRegulatorySummary = {
  pathwayId: string;
  pathwayName: string;
  totalGenes: number;
  totalReactions: number;
  totalRegulators: number;
  genes: PathwayRegulatoryGene[];
  regulators: PathwayRegulator[];
  explanation: string;
};
...
function collectNodes(graph: any): any[] {
  ...
  if (Array.isArray(graph.elements)) return graph.elements.filter((item: any) => item.group === "nodes" || item.data?.id);
  return [];
}
...
function edgeConfidence(edge: any): number {
  const parsed = Number(callOrRead(edge, "confidence") ?? callOrRead(edge, "confidenceScore"));
  return Number.isFinite(parsed) ? clamp(parsed) : 0;
}

function findPathwayMetadata(pathwayIdOrName: string): { pathwayId: string; pathwayName: string } {
  const query = normalizeGeneId(pathwayIdOrName);
  const match = getAllMetabolicPathways().find((pathway: any) => {
...
export function getGenesForPathway(pathwayIdOrName: string): PathwayRegulatoryGene[] {
  if (!pathwayIdOrName) return [];
  const byGene = new Map<string, PathwayRegulatoryGene>();

  for (const gene of getAdapterGenesForPathway(pathwayIdOrName) || []) {
    ...
    const seenReactions = new Set(entry.reactions.map(r => r.reactionId));
    for (const reaction of gene.reactions || []) {
      const reactionId = reaction.reactionId || reaction.id;
      if (!reactionId || seenReactions.has(reactionId)) continue;
...
export function getRegulatorsForPathway(graph: RegulatoryGraph, pathwayIdOrName: string): PathwayRegulator[] {
  const genes = getGenesForPathway(pathwayIdOrName);
  const pathwayGenes = new Set(genes.map(g => normalizeGeneId(g.geneId)));
  ...
  const grouped = new Map<string, { genes: Set<string>; types: Set<string>; confidences: number[] }>();
  for (const edge of collectEdges(graph)) {
    const target = normalizeGeneId(edgeTarget(edge));
    if (!pathwayGenes.has(target)) continue;
    const source = normalizeGeneId(edgeSource(edge));
    if (!source) continue;
    const bucket = grouped.get(source) || { genes: new Set<string>(), types: new Set<string>(), confidences: [] };
  ...
  return Array.from(grouped.entries()).map(([tfId, bucket]) => {
    ...
    const regulatorScore = clamp(
      0.50 * clamp(bucket.genes.size / 20) +
      0.30 * averageConfidence +
      0.20 * regulationDiversityScore
    );
...
export function generatePathwayRegulatoryExplanation(summary: PathwayRegulatorySummary): string {
...
export function getPathwayRegulatorySummary(graph: RegulatoryGraph, pathwayIdOrName: string): PathwayRegulatorySummary {
  ...
  const summary: PathwayRegulatorySummary = {
