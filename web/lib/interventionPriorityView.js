...
(function () {
    ...
    function shell(container) {
        ...
        <div class="ip-header"><div><h2><i class="fa-solid fa-bullseye" style="color:#4338ca"></i> Cross-module target priorities</h2><p>Rank targets by regulatory evidence, systems impact, engineering tractability and risk across iron, oxygen, carbon, nitrogen and stress modules. Rankings guide follow-up; they do not prescribe a perturbation without a product objective.</p></div><div style="display:flex; gap:8px;"><button class="secondary-btn" id="ip-help-toggle" style="padding:4px 10px; font-size:11px;"><i class="fa-solid fa-circle-question"></i> Parameter Guide</button><button class="secondary-btn" id="ip-back"><i class="fa-solid fa-arrow-left"></i> Gene Explorer</button></div></div>
        <div class="ip-help-banner" id="ip-help-banner" style="background:#f0f9ff; border:1px solid #bae6fd; border-radius:8px; padding:10px 12px; margin-bottom:12px; font-size:11px; color:#0c4a6e; display:none;">
            <div style="font-weight:700; color:#0369a1; margin-bottom:6px; display:flex; justify-content:space-between; align-items:center;">
                <span><i class="fa-solid fa-circle-info"></i> Parameter Definitions (跨模块靶点工程优先级参数说明)</span>
                <button id="ip-help-close" style="border:none; background:none; color:#0369a1; cursor:pointer; font-size:12px;"><i class="fa-solid fa-xmark"></i></button>
            </div>
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap:8px; font-size:10.5px; line-height:1.4;">
                <div><strong>Grade (证据等级):</strong> 综合实验评级 (A: 极高信度多重验证, B: 高信度, C: 中等, D: 低)。</div>
                <div><strong>Modules (调控模块):</strong> 基因跨越参与的条件调控模块数 (Carbon, Iron, Oxygen, Nitrogen, Stress)。</div>
                <div><strong>Evidence (证据分):</strong> 结合文献、ChIP-seq 结合峰、表达谱及 iModulons 的多维定量支撑得分 (0-1.0)。</div>
                <div><strong>Impact (系统影响分):</strong> 在代谢与转录网络中的系统级影响力 (基于网络中心性与下游调控范围)。</div>
                <div><strong>Risk (风险分):</strong> 敲除或干预导致的细胞生长抑制或致死风险 (结合已知必需基因库与拓扑阻断风险)。</div>
                <div><strong>Priority (综合优先级):</strong> 综合推荐得分 (Priority = Evidence + Impact + Tractability - Risk)，得分越高越推荐。</div>
                <div><strong>Strategy (工程策略):</strong> 推荐干预策略 (如多应激控制节点、代谢干预、需精确滴定、动态微调等)。</div>
            </div>
        </div>
    ...
    function render() {
        ...
        <div class="ip-layout"><section class="ip-panel ip-table-wrap"><table class="ip-table"><thead><tr><th title="Target Gene Symbol and Locus Tag">Target</th><th title="Evidence Grade (A: High confidence ... D: Low confidence)">Grade <i class="fa-solid fa-circle-question" style="font-size:8.5px; opacity:0.6;"></i></th><th title="Number and list of regulated modules (Iron, Oxygen, Carbon, Nitrogen, Stress)">Modules <i class="fa-solid fa-circle-question" style="font-size:8.5px; opacity:0.6;"></i></th><th title="Evidence Score: Combined score from literature, ChIP-seq, expression & iModulons">Evidence <i class="fa-solid fa-circle-question" style="font-size:8.5px; opacity:0.6;"></i></th><th title="Systems Impact Score: Network centrality and pathway downstream scope">Impact <i class="fa-solid fa-circle-question" style="font-size:8.5px; opacity:0.6;"></i></th><th title="Risk Score: Growth perturbation or essentiality risk">Risk <i class="fa-solid fa-circle-question" style="font-size:8.5px; opacity:0.6;"></i></th><th title="Integrated Priority Score: Final target priority recommendation score">Priority <i class="fa-solid fa-circle-question" style="font-size:8.5px; opacity:0.6;"></i></th><th title="Engineering Strategy Class recommendation">Strategy <i class="fa-solid fa-circle-question" style="font-size:8.5px; opacity:0.6;"></i></th></tr></thead><tbody>${state.targets.map(row).join('')||'<tr><td colspan="8">No targets match.</td></tr>'}</tbody></table></section><aside class="ip-panel ip-detail" id="ip-detail">Select a target to inspect its score decomposition and module evidence.</aside></div>`;
    ...
    function bind(){
        document.getElementById('ip-back').addEventListener('click',()=>window.setActiveWorkflowEntry?.('gene'));
        document.getElementById('ip-apply').addEventListener('click',load);
        document.getElementById('ip-query').addEventListener('keydown',e=>{if(e.key==='Enter')load();});
        const helpBtn = document.getElementById('ip-help-toggle');
        const helpBanner = document.getElementById('ip-help-banner');
        const helpClose = document.getElementById('ip-help-close');
        if(helpBtn && helpBanner){
            helpBtn.addEventListener('click', ()=>{
                helpBanner.style.display = helpBanner.style.display === 'none' ? 'block' : 'none';
            });
        }
        if(helpClose && helpBanner){
