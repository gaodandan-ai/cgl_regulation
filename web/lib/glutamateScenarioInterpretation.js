(function (global) {
    function generateGlutamateScenarioInterpretation(result, selectedReaction) {
        ...
        if (result.trackedFluxes && Array.isArray(result.trackedFluxes)) {
            const tf = result.trackedFluxes.find(f => f.reactionId === reactionId);
            if (tf) {
    ...
    global.glutamateScenarioInterpretation = {
        generateGlutamateScenarioInterpretation
    };
})(window);
