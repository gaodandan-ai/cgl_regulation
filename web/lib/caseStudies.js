(function (global) {
    ...
    function generateCaseStudyNarrative(input) {
    ...
    function runCaseStudy(caseStudyId, graph) {
        ...
        try {
            const qualityLib = global.analysisQuality;
            const pathwayLib = global.pathwayRegulatoryView;
            const engineeringLib = global.candidateEngineeringTargets;

            const qual = qualityLib ? qualityLib.getMetabolicMappingCoverage(graph) : null;
            const enz = qualityLib ? qualityLib.getEcCGL1CoverageSummary(graph) : null;
            ...
            if (caseStudy.id === "glutamate-regulation") {
                if (pathwayLib) {
                    results.pathwaySummary = pathwayLib.getPathwayRegulatorySummary(graph, "glutamate");
                ...
                if (pathwayLib) {
                    results.pathwaySummary = pathwayLib.getPathwayRegulatorySummary(graph, "tca");
                    if (results.pathwaySummary && results.pathwaySummary.regulators) {
                        results.tfRanking = results.pathwaySummary.regulators;
                    }
                }
                ...
                if (engineeringLib) {
                    results.engineeringCandidates = engineeringLib.findEngineeringTargetCandidates(graph, {
    ...
    global.caseStudies = {
        BUILT_IN_CASE_STUDIES,
        runCaseStudy,
        generateCaseStudyNarrative
    };
})(window);
