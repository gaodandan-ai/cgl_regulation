export type GlutamateReactionClass = "exchange" | "export" | "transport" | "biosynthesis" | "consumption" | "uncertain";

export type GlutamateCandidate = {
  reactionId: string;
  name: string;
  equation: string;
  lowerBound: number;
  upperBound: number;
  classification: GlutamateReactionClass;
  confidence: "high" | "medium" | "low";
  reason: string;
};

export type GlutamateState = {
  selectedGlutamateReactionId: string | null;
  selectedGlutamateReactionClass: GlutamateReactionClass | null;
  userVerified: boolean;
};

export function classifyGlutamateReaction(reaction: any): {
  classification: GlutamateReactionClass;
  confidence: "high" | "medium" | "low";
  reason: string;
} {
  ...
  if (isExchange) {
    ...
    } else if (name.includes("decarboxylase") || name.includes("kinase") || name.includes("synth synthase")) {
      return {
